 ...
 * Now when the handlers {@link #handle(StringBuilder, Player, Map, HTMLTemplateFunc[])} method is called, contents will contain "template2.tmpl"<br>
 * @author HorridoJoho
 */
public abstract class HTMLTemplateFunc
{
	...
	/** flag to determine if the template function needs processing of it's contents before contents are passed to the {@link #handle(StringBuilder, Player, Map, HTMLTemplateFunc[])} method */
	 ...
	 * @param requiresPreprocessing flag to determine if the template function needs processing of it's contents before contents are passed to the {@link #handle(StringBuilder, Player, Map, HTMLTemplateFunc[])}
	 */
	...
	/**
	 * @return true when the handler needs the contents preprocessed by the template engine before it is passed to the {@link #handle(StringBuilder, Player, Map, HTMLTemplateFunc[])} metod, false otherwise
	 */
	 ...
	 */
	public abstract Map<String, HTMLTemplatePlaceholder> handle(StringBuilder content, Player player, Map<String, HTMLTemplatePlaceholder> placeholders, HTMLTemplateFunc[] funcs);
}